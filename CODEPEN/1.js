...
const ReviewHighlighter = ({ review }) => {
    ...
    const getHighlightedText = () => {
        ...
        review.analytics.forEach((item, i) => {
            const { sentiment, highlight_indices } = item;
            highlight_indices.forEach(([start, end]) => {
                if (start > lastIndex) {
                    elements.push(review.content.substring(lastIndex, start));
                }
                elements.push(
        ...
        if (lastIndex < review.content.length) {
            elements.push(review.content.substring(lastIndex));
        }
    ...
    const [bookmarked, setBookmarked] = React.useState(
        JSON.parse(localStorage.getItem(`bookmark-${review.review_id}`)) || false
    );

    const toggleBookmark = () => {
        setBookmarked(!bookmarked);
        localStorage.setItem(`bookmark-${review.review_id}`, JSON.stringify(!bookmarked));
    };
...
const ReviewList = () => {
    ...
    const reviewsPerPage = 5; // Change this to increase/decrease number of reviews per page
    ...
    const sortReviews = (reviews) => {
        switch (sortOption) {
            ...
            case 'rating':
                return reviews.sort((a, b) => b.rating_review_score - a.rating_review_score);
            case 'sentiment':
                return reviews.sort((a, b) => a.sentiment && b.sentiment ? a.sentiment.localeCompare(b.sentiment) : 0);
            default:
                return reviews;
        }
    };
    ...
    const filterReviews = (reviews) => {
        if (filterOption === 'all') return reviews;
        return reviews.filter(review => 
            review.sentiment && review.sentiment.toLowerCase() === filterOption
        );
    };
    ...
    const paginate = (reviews) => {
        const indexOfLastReview = currentPage * reviewsPerPage;
        const indexOfFirstReview = indexOfLastReview - reviewsPerPage;
        return reviews.slice(indexOfFirstReview, indexOfLastReview);
    };

    const filteredAndSortedReviews = searchReviews(filterReviews(sortReviews([...reviewsData])));
    const paginatedReviews = paginate(filteredAndSortedReviews);
    ...
    return (
        <div className="review-list-container">
            ...
            <div className="pagination">
                {[...Array(Math.ceil(filteredAndSortedReviews.length / reviewsPerPage)).keys()].map((number) => (
